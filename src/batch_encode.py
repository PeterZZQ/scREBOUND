...
# def tokenize_batch_feats(batch_feats, use_mito = True, use_tech = False, use_nmeasure = False, expr_binsize = 1):
#     """
#     Description:
#     -------------
...
#     if use_tech:
#         batch_info = ["assay", "suspension_type"]
#         assert "assay" in batch_feats.columns
#         assert "suspension_type" in batch_feats.columns
#         nbucket_dict["assay"] = len(assay_codebook)
#         nbucket_dict["suspension_type"] = len(suspension_codebook)
#     else:
#         batch_info = []
...
#     if use_nmeasure:
#         batch_info += ["n_measures"]
#         assert "n_measures" in batch_feats.columns
        
#     if use_mito:
#         batch_info += ["prop_mito"]

#     batch_feats = batch_feats[batch_info + batch_gene_name]
#     # now tokenize the batch_feats
#     batch_feats_digitize = pd.DataFrame(data = 0, index = batch_feats.index.values, columns = batch_info + batch_gene_name)
...
#     libsize_log = np.log1p(batch_feats["libsize"].values.squeeze())
#     raw_mean_nnz_log = np.log1p(batch_feats["raw_mean_nnz"].values.squeeze())

#     # value features:
#     libsize_bucket = np.arange(0, 15.0, 1)
#     batch_feats_digitize["libsize"] = np.digitize(libsize_log, libsize_bucket, right = True)
#     nbucket_dict["libsize"] = len(libsize_bucket) + 1

#     nnz_bucket = np.arange(0, 1, 0.1)       
#     batch_feats_digitize["nnz"] = np.digitize(batch_feats["nnz"], nnz_bucket, right = True)
#     nbucket_dict["nnz"] = len(nnz_bucket) + 1
    
#     expr_bucket = torch.arange(0, 10.0, expr_binsize)
#     batch_feats_digitize["raw_mean_nnz"] = np.digitize(raw_mean_nnz_log, expr_bucket, right = True)
#     nbucket_dict["raw_mean_nnz"] = len(expr_bucket) + 1
    
#     if use_nmeasure:
#         # upper bound 70000
#         n_measures_bucket = np.arange(0, 1, 0.1) 
#         n_measures_norm = batch_feats["n_measures"].values/70000
#         batch_feats_digitize["n_measures"] = np.digitize(n_measures_norm, n_measures_bucket, right = True)
#         nbucket_dict["n_measures"] = len(n_measures_bucket) + 1

#     if use_mito:
#         prop_mito_bucket = np.arange(0, 1, 0.1)
#         batch_feats_digitize["prop_mito"] = np.digitize(batch_feats["prop_mito"], prop_mito_bucket, right = True)
#         nbucket_dict["prop_mito"] = len(prop_mito_bucket) + 1

#     batch_feats_digitize[batch_gene_name] = np.digitize(batch_feats[batch_gene_name], expr_bucket, right = True)
#     for gene in batch_gene_name:
#         nbucket_dict[gene] = len(expr_bucket) + 1

#     num_buckets = [nbucket_dict[x] for x in batch_feats_digitize.columns]
...
# def tokenize_batch_gene(batch_feats, max_vals = None, margin = 0.2, nbins = 10, normalize = True):
#     # select the batch related genes
#     batch_feats_genes = batch_feats[batch_gene_name]

#     # determine binsize according to max value of each gene
#     if max_vals is None:
#         if normalize:
#             max_vals = np.max(batch_feats_genes.values, axis = 0) + margin
#         else:
#             max_vals = np.array([np.max(batch_feats.values) + margin] * batch_feats.shape[1])

#     else:
#         # upper bound the values
#         for idx, col in enumerate(batch_feats_genes.columns):
#             batch_feats_genes.loc[batch_feats_genes[col].values.squeeze() > max_vals[idx], col] = max_vals[idx]
#     min_vals = np.zeros_like(max_vals)

#     # min-max normalization, easiler for binning
#     batch_feats_genes_norm = (batch_feats_genes.values - min_vals[None, :])/(max_vals[None, :] - min_vals[None, :])

#     # 10 bins: 9 digits and 0->0 [0, 1, ..., 10], make sure all bins has training data
#     bins = np.arange(0, 1, 1/(nbins - 1))
#     batch_feats_digit = np.digitize(batch_feats_genes_norm, bins = bins, right = True)    
#     batch_feats_digit = pd.DataFrame(index = batch_feats_genes.index, columns = batch_feats_genes.columns, data = batch_feats_digit)
#     return batch_feats_digit, max_vals
