...
def infer_databatch(model, data_sample, multigpus: bool = True):
    ...
    loss = model_acc.model_config.lamb_mlm * loss_mlm + model_acc.model_config.lamb_mincut * (l_mincut + 0.01 * l_ortho) + model_acc.model_config.lamb_sup * loss_sup

    return loss, {"mlm": loss_mlm.item(), "mincut": l_mincut.item(), "ortho": l_ortho.item(), "metric": loss_sup.item()}
