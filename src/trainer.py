...
def infer_databatch(model, data_sample, multigpus: bool = True):
        ...
        if label_sample is not None:
            if model_acc.model_config.sup_type == "classifier":
                ...
                # TODO: find unknown labels
                mask_samples = ~(label_sample == model_acc.label_mask)
                ce = nn.CrossEntropyLoss()
                loss_sup = ce(label_pred[mask_samples], label_sample[mask_samples])
            
            elif model_acc.model_config.sup_type == "classifier-bincode":
                label_pred = model_acc.classifier(cell_embed)
                # NOTE: label_sample is (n_batch, n_class)
                # TODO: find unknown labels
                ...
                elif model_acc.model_config.sup_type == "contrastive-cb":
        ...
        if (model_acc.model_config.use_discriminator) & (model_acc.model_config.sup_type == "classifier-bincode"):
            # reverse the gradient of the model
            cell_embed = gradient_reversal(cell_embed, model_acc.model_config.lamb_reverse)
            # pass the reversed gradient into discriminator
            batch_pred = model_acc.discriminator(cell_embed, label_sample)
            ce = nn.CrossEntropyLoss()
...
def cell_embed(model, dataloader, mask_prob: float = 0.0, multi_gpus = True):
    ...
    with torch.no_grad():
        for data_sample in tqdm.tqdm(dataloader, desc=f"Calc embed"):
            ...
            if model_acc.model_config.sup_type == "classifier":
                label_pred = model_acc.classifier(cell_embed)
                labels_pred.append(label_pred.squeeze(0).detach().cpu().numpy())
            else:
                labels_pred.append(np.array([np.nan] * cell_embed.shape[0]))            
    ...
    meta = pd.DataFrame.from_dict({"labels": labels, "batchs": batchs, "labels_pred": labels_pred})
